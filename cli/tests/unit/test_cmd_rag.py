...
from pdf_autofillr_cli.cmd_rag import (
    _load_category, _predict, _feedback, _metrics, _error_analytics,
)
...
class TestLoadCategory:
    def test_empty_string(self):          assert _load_category("") == {}
    def test_empty_json(self):            assert _load_category("{}") == {}
    def test_inline_json(self):           assert _load_category('{"category":"Finance"}') == {"category": "Finance"}
    def test_file_path(self, tmp_path):
        f = tmp_path / "cat.json"
        f.write_text('{"category":"PM"}')
        assert _load_category(str(f)) == {"category": "PM"}
    def test_invalid_json_raises(self):
        with pytest.raises(json.JSONDecodeError): _load_category("bad-json")
...
class TestRagPredict:
    def test_predict_calls_client(self, tmp_json, mock_rag_client):
        ...
        args = argparse.Namespace(user="u1", session="s1", pdf="p1", fields=fp, hash="abc", category=cp)
    ...
    def test_unwraps_wrapped_fields(self, tmp_json, mock_rag_client):
        ...
        args = argparse.Namespace(user="u1", session="s1", pdf="p1", fields=fp, hash="abc", category="{}")
        _predict(args, mock_rag_client)
...
class TestRagFeedback:
    def test_feedback_calls_client(self, tmp_json, mock_rag_client):
        ...
        mock_rag_client.submit_feedback.assert_called_once_with("u1", "s1", "p1", [{"error_type": "x"}])
...
class TestRagMetrics:
    def test_metrics_global(self, mock_rag_client):
        args = argparse.Namespace(func=None, command="rag", rag_command="metrics",
                                  metric_type="global", user=None, session=None, pdf=None,
                                  category=None, subcategory=None, doctype=None, pdf_hash=None)
    ...
    def test_metrics_per_pdf(self, mock_rag_client):
        args = argparse.Namespace(func=None, command="rag", rag_command="metrics",
                                  metric_type="pdf", user="u1", session="s1", pdf="p1",
                                  category=None, subcategory=None, doctype=None, pdf_hash=None)
        _metrics(args, mock_rag_client)
